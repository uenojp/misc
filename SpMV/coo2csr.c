...
int coo2csr(const uint32_t* coo_row_index, const uint32_t* coo_col_index, const double* coo_value,
            uint32_t** csr_row_ptr, uint32_t** csr_col_index, double** csr_value, uint32_t n,
            uint32_t nnz) {
    ...
    *csr_row_ptr = (uint32_t*)malloc((size_t)(n + 1) * sizeof(uint32_t));
    *csr_col_index = (uint32_t*)malloc((size_t)nnz * sizeof(uint32_t));
    *csr_value = (double*)malloc((size_t)nnz * sizeof(double));
    if (!(*csr_row_ptr) || !(*csr_col_index) || !(*csr_value)) {
        return -1;
    }
    ...
    for (uint32_t i = 0; i < nnz; i++) {
        ...
        (*csr_row_ptr)[coo_row_index[i] + 1]++;
        (*csr_col_index)[i] = coo_col_index[i];
        (*csr_value)[i] = coo_value[i];
    }
