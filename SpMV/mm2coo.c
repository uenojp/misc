...
int mm2coo(const char* pathname, uint32_t** coo_row_index_ptr, uint32_t** coo_col_index_ptr,
           double** coo_value_ptr, uint32_t* nptr, uint32_t* nnzptr) {
    FILE* fp = NULL;
    ...
    *coo_row_index_ptr = (uint32_t*)malloc((size_t)nnz * sizeof(uint32_t));
    *coo_col_index_ptr = (uint32_t*)malloc((size_t)nnz * sizeof(uint32_t));
    *coo_value_ptr = (double*)malloc((size_t)nnz * sizeof(double));
    if (!(*coo_row_index_ptr) || !(*coo_col_index_ptr) || !(*coo_value_ptr)) {
        return -1;
    }

    if (read_mm_data(fp, *coo_row_index_ptr, *coo_col_index_ptr, *coo_value_ptr, nnz) < 0) {
