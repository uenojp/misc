use rayt::{render_aa, Camera, Color, Float3, Point3, Ray, Scene, Vec3};
...
impl Metal {
    const fn new(albedo: Float3) -> Self {
        Self { albedo }
    }
}
...
impl SimpleScene {
    fn new() -> Self {
        ...
        world.push(Box::new(Sphere::new(
            Point3::new(-1.0, 0.0, -1.0),
            0.5,
            Metal::new(Float3::new(0.8, 0.8, 0.8)),
        )));
        world.push(Box::new(Sphere::new(
    ...
    fn trace(&self, ray: &Ray) -> Color {
        ...
        if let Some(hit) = hit_info {
            if let Some(scatter) = hit.m.scatter(ray, &hit) {
                scatter.albedo * self.trace(&scatter.ray)
            } else {
                Color::zero()
            }
        } else {
